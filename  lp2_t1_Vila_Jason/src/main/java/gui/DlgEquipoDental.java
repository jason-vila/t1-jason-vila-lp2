...
import model.DentistaJasonVC;
import model.EquipoDentalJasonVC;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		...
		try {
			String jpql = "select d from DentistaJasonVC d";
			List<DentistaJasonVC> lstDentista = manager.createQuery(jpql, DentistaJasonVC.class).getResultList();

			for (DentistaJasonVC dentista : lstDentista) {
	...
	void listar() {
		...
		String jpql = "select ed, d.nombre_completo from EquipoDentalJasonVC ed join ed.dentista d";
		...
		try {
			...
			for (Object[] objects : resultados) {
				EquipoDentalJasonVC ed = (EquipoDentalJasonVC) objects[0];
				String nombreDentista = (String) objects[1];
				
				imprimir("Nro Equipo..........: "+ ed.getNro_equipo());
				imprimir("Nombre..............: "+ ed.getNombre());
				imprimir("Costo...............: "+ ed.getCosto());
				imprimir("Fecha de adquisición: "+ ed.getFecha_adquisicion());
				imprimir("Dentista............: "+ ed.getDentista().getCop() + " - " + nombreDentista);
				imprimir("Correo..............: "+ ed.getDentista().getCorreo());
				imprimir("Especialidad........: "+ ed.getDentista().getEspecialidad());
				imprimir("Estado..............: "+ ed.getEstado());
				imprimir("------------------------------------------------------");
			}
		} finally {
			manager.close();
	...
	void adicionar() {
		...
		String estadoStr = (String) cboEstados.getSelectedItem();
		char estado = estadoStr.charAt(0);
		DentistaJasonVC dentista = (DentistaJasonVC) cboDentistas.getSelectedItem();
		
		EquipoDentalJasonVC ed = new EquipoDentalJasonVC();
		ed.setNombre(nombre);
		ed.setCosto(costo);
		ed.setEstado(estado);
		ed.setDentista(dentista);
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Se ha registrado un equipo dental", "Exitoso", JOptionPane.INFORMATION_MESSAGE);
			limpiar();
		} finally {
			manager.close();
		}
	...
	void buscar() {
		int nro = Integer.parseInt(txtNroEquipo.getText()); 
		
		EntityManager manager = JPAUtil.getEntityManager();
		
		try {
			EquipoDentalJasonVC ed = manager.find(EquipoDentalJasonVC.class, nro);
			
			if (ed == null) {
				JOptionPane.showMessageDialog(null, "No existe el equipo dental", "ERROR", JOptionPane.ERROR_MESSAGE);
				return;
			}
			
			txtNombre.setText(ed.getNombre()+"");
			txtCosto.setText(ed.getCosto()+"");
			txtFechaAdquisicion.setText(ed.getFecha_adquisicion()+"");
			cboEstados.setSelectedItem(ed.getEstado());
			cboDentistas.setSelectedItem(ed.getDentista());
